...
export class ProductRead2DataSource extends DataSource<Product> {
  ...
  private getSortedData(data: Product[]): Product[] {
    ...
    return data.sort((a, b) => {
      ...
      // Use the optional chaining operator to handle possible undefined values
      const aValue = a[this.sort?.active as keyof Product] as string | number;
      const bValue = b[this.sort?.active as keyof Product] as string | number;
